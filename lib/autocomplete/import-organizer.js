...
class ImportOrganizer {
  ...
  constructImports(suggestion) {
    ...
    return [].concat(
      formattedImports.filter((fi) => fi.type === LIBRARY_IMPORT).map(fi => fi.importText).join('\n'),
      '',
      formattedImports.filter((fi) => fi.type === PROJECT_IMPORT).map(fi => fi.importText).join('\n')
    ).join('\n');
