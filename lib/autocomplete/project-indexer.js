  ...
  try {
    ...
    if(list && list.length > 0) {
      for(let item of list) {
        ...
        if (stats.isFile() && item === 'index.ts' ) { // && item.slice(-3) === '.ts' && item.slice(-8) !== '.spec.ts'
          files.push(fullPath);
    ...
    if (node.kind === ts.SyntaxKind.ExportDeclaration) {
      if (node.exportClause) {
        ...
        for(let elem of node.exportClause.elements){
          ast.push({ text: elem.name.text, type: 'class', source: specifierPath, sourceType: 'app' });
        }
      } else {
        ast = ast.concat(parseAst(path.join(path.dirname(filePath), node.moduleSpecifier.text)));
      }
    } else if (declarationTypes.indexOf(node.kind) !== -1) {
      if (node.modifiers && node.modifiers[0].kind === ts.SyntaxKind.ExportKeyword) {
        const specifierPath = filePath; // path.join(path.dirname(filePath), path.basename(filePath, '.ts'));
        ast.push({ text: node.name.text, type: 'class', source: specifierPath, sourceType: 'app' });
      }
    }
  });
