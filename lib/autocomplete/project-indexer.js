  ...
  ts.forEachChild(sourceFile, (node) => {
    if (node.kind === ts.SyntaxKind.ExportDeclaration) {
      if (node.exportClause) {
        const specifierPath = path.join(filePath, node.moduleSpecifier.text);
        for(let elem of node.exportClause.elements){
          ast.push({text: elem.name.text, type: 'class', source: specifierPath});
        }
      } else {
      ...
      if (node.modifiers && node.modifiers[0].kind === ts.SyntaxKind.ExportKeyword) {
        ...
        ast.push({text: node.name.text, type: 'class', source: specifierPath});
      }
    }
  });
