...
class ImportOrganizer {
  ...
  constructImports(suggestion) {
    ...
    // NOTE: Very bad implementation. REFACTOR
    const groupedImports = [];
    groupedImports.push(formattedImports.filter((fi) => fi.type === LIBRARY_IMPORT).map(fi => fi.importText).join('\n'))
    if (groupedImports.length > 0 && groupedImports[0] !== '') {
      groupedImports.push('');
    } else {
      groupedImports.pop();
    }
    groupedImports.push(formattedImports.filter((fi) => fi.type === PROJECT_IMPORT).map(fi => fi.importText).join('\n'))
